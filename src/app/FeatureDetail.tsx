    ...
    <div className="rounded-[2rem] bg-[#09244B] p-4 shadow-[0_30px_80px_rgba(9,36,75,0.22)] sm:p-6">
      <div className="overflow-hidden rounded-[1.4rem] bg-white">
        <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
          <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#FFF1E6] text-[#FF8000]"><Icon className="h-5 w-5" /></span>
      ...
      <main>
          ...
          <div className="relative mx-auto max-w-6xl">
            ...
            <div className="grid items-center gap-14 lg:grid-cols-[1.02fr_0.98fr] lg:gap-20">
              <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }}>
                <div className="inline-flex items-center gap-2 rounded-full border border-[#FF8000]/20 bg-white px-4 py-2 text-sm font-semibold text-[#B95700] shadow-sm"><Icon className="h-4 w-4" />{page.eyebrow}</div>
        ...
        <section className="px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
          <div className="mx-auto max-w-6xl">
            <Reveal className="max-w-3xl"><p className="text-sm font-bold uppercase tracking-[0.2em] text-[#D96900]">Core capabilities</p><h2 className="mt-4 text-3xl font-bold sm:text-5xl">Everything the workflow needs, kept in context.</h2></Reveal>
              ...
              {page.workflow.map((item, index) => (
                <Reveal key={item.step} delay={index * 0.06}>
                  <div className="grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-6 sm:grid-cols-[56px_1fr]">
                    <span className="text-2xl font-bold text-[#FF9B3D]">{item.step}</span><div><h3 className="text-lg font-bold">{item.title}</h3><p className="mt-2 leading-7 text-white/65">{item.description}</p></div>
                  </div>
                </Reveal>
              ))}
            </div>
        ...
        <section className="bg-[#F7FCFD] px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
          <div className="mx-auto max-w-6xl">
            <Reveal className="mx-auto max-w-3xl text-center"><p className="text-sm font-bold uppercase tracking-[0.2em] text-[#D96900]">What this improves</p><h2 className="mt-4 text-3xl font-bold sm:text-5xl">A more connected way to run the work.</h2></Reveal>
            <div className="mt-12 grid gap-5 md:grid-cols-3">
              {page.outcomes.map((outcome, index) => <Reveal key={outcome} delay={index * 0.06}><div className="flex h-full gap-4 rounded-2xl bg-white p-6 shadow-[0_8px_28px_rgba(9,36,75,0.06)]"><Check className="mt-1 h-5 w-5 shrink-0 text-[#FF8000]" /><p className="font-semibold leading-7">{outcome}</p></div></Reveal>)}
            </div>
          </div>
        </section>
